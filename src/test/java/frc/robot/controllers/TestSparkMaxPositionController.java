...
public class TestSparkMaxPositionController {
    ...
    public void testInvertedAxis() {
        ...
        SparkMaxPositionController sm = new SparkMaxPositionController(mockMotor2, config2, lowerLimit, upperLimit,
                encoder);
    ...
    // when a position is requested, but then a command runs that sets the speed
    // manually,
    ...
    public void testUsingSpeedControlInvalidatesRequestedPosition() {
        ...
        // assertEquals(RobotConstants.INDICATOR_VALUES.POSITION_NOT_SET,
        // c.getRequestedPosition());
    ...
    protected void assertPositionAndState(SparkMaxPositionController c, int expectedPosition,
            HomingState expectedState) {
        assertEquals(expectedPosition, c.getActualPosition(), COMPARE_TOLERANCE);
        assertEquals(expectedState, c.getMotionState());
    }

    protected void assertPositionAndState(SparkMaxPositionController c, double expectedPosition,
            HomingState expectedState) {
        assertEquals(expectedPosition, c.getActualPosition(), COMPARE_TOLERANCE);
        assertEquals(expectedState, c.getMotionState());
    }
