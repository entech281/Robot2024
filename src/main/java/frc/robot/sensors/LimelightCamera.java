...
/**
 * The {@code LimelighCamera} class contains fields and methods pertaining to
 * the function of the Limelight camera.
 */
public class LimelightCamera implements /* PIDSource, */ ICamera {
	...
	// The 2020 target has an overall height of 1 ft. 5 in. (~43 cm), and a width of
	// 3 ft. 3¼ in. (~100 cm).
	...
	public static final double DEFAULT_OFFSET_CAMERA_TARGET_INCHES = 10; // we need to leave some space between the
	...
	private void updateFromNT() {
		...
		do {
			...
			if (tv != 0.0) { // if there is at least one valid target
				...
				double[] width = { thor };
				double[] height = { tvert };
				double[] centerX = { tx * HORIZONTAL_CAMERA_RES_PIXELS / HORIZONTAL_FOV_DEGREES
						+ (HORIZONTAL_CAMERA_RES_PIXELS / 2) }; // to use same units as HMCamera
				double[] centerY = {
						ty * VERTICAL_CAMERA_RES_PIXELS / VERTICAL_FOV_DEGREES + (VERTICAL_CAMERA_RES_PIXELS / 2) }; // to
																														// use
																														// same
																														// units
																														// as
	...
	// The PIDController that will call this method runs in a different thread than
	// the scheduler thread,
	// so it is important that this class be made thread-safe. This is why all the
	...
	public synchronized double pidGet() {
		...
		return -getPixelDisplacementToCenterToCompositeTarget(); // we are located at the opposite or the displacement
																	// we need to shift by
	}
	...
	public synchronized double pidGet2() {
		...
		// double distanceToTargetReportedByCamera =
		// getDistanceToCompositeTargetUsingHorizontalFov();
	...
	public synchronized void setOffsetBetweenCameraAndTarget(double offset) {
		offsetCameraTargetInches = Math.max(Math.min(offset, MAX_OFFSET_CAMERA_TARGET_INCHES),
				MIN_OFFSET_CAMERA_TARGET_INCHES);
	}
