...
{
  List<SwerveMotor> createdMotors = new ArrayList<SwerveMotor>();
  ...
  public SwerveMotor createMotor(boolean isDriveMotor)
  {
    switch (type)
    {
      ...
      case "neo":
      case "sparkmax":
        createdMotors.add(new SparkMaxSwerve(id, isDriveMotor));
        return createdMotors.get(createdMotors.size() - 1);
      case "falcon":
      case "talonfx":
        createdMotors.add(new TalonFXSwerve(id, canbus != null ? canbus : "", isDriveMotor));
        return createdMotors.get(createdMotors.size() - 1);
      case "talonsrx":
        createdMotors.add(new TalonSRXSwerve(id, isDriveMotor));
        return createdMotors.get(createdMotors.size() - 1);
      default:
        throw new RuntimeException(type + " is not a recognized absolute encoder type.");
    }
